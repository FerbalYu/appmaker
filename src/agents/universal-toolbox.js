      ...
      if (filePattern === '*' || entry.name.match(new RegExp(filePattern.replace('*', '.*')))) {
        try {
          ...
          lines.forEach((line, idx) => {
            if (regex.test(line)) {
              results.push({ file: path.relative(workspaceRoot, fullPath), line: idx + 1, content: line.trim() });
            }
          });
        } catch {}
      }
...
export class UniversalToolbox {
  ...
  _registerFileTools() {
    this._registerTool('read_file', TOOL_CATEGORIES.FILE_SYSTEM, '读取单个文件内容', {
      type: 'object',
      properties: {
    ...
    this._registerTool('read_multiple_files', TOOL_CATEGORIES.FILE_SYSTEM, '批量读取多个文件', {
      type: 'object',
      properties: {
    ...
    this._registerTool('write_file', TOOL_CATEGORIES.FILE_SYSTEM, '写入文件内容', {
      type: 'object',
      properties: {
    ...
    this._registerTool('create_directory', TOOL_CATEGORIES.FILE_SYSTEM, '创建目录', {
      type: 'object',
      properties: {
    ...
    this._registerTool('delete_file', TOOL_CATEGORIES.FILE_SYSTEM, '删除文件或目录', {
      type: 'object',
      properties: {
        path: { type: 'string' },
    ...
    this._registerTool('move_file', TOOL_CATEGORIES.FILE_SYSTEM, '移动文件或目录', {
      type: 'object',
      properties: {
        source: { type: 'string' },
    ...
    this._registerTool('copy_file', TOOL_CATEGORIES.FILE_SYSTEM, '复制文件或目录', {
      type: 'object',
      properties: {
        source: { type: 'string' },
        destination: { type: 'string' }
    ...
    this._registerTool('list_directory', TOOL_CATEGORIES.FILE_SYSTEM, '列出目录内容', {
      type: 'object',
      properties: {
    ...
    this._registerTool('file_exists', TOOL_CATEGORIES.FILE_SYSTEM, '检查文件或目录是否存在', {
      type: 'object',
      properties: {
        path: { type: 'string' }
      },
    ...
    this._registerTool('get_file_info', TOOL_CATEGORIES.FILE_SYSTEM, '获取文件详细信息', {
      type: 'object',
      properties: {
        path: { type: 'string' }
      },
    ...
    this._registerTool('search_files', TOOL_CATEGORIES.FILE_SYSTEM, '在文件中搜索内容', {
      type: 'object',
      properties: {
        pattern: { type: 'string' },
        dir_path: { type: 'string', default: '.' },
      ...
      await searchInDir(searchDir, args.pattern, args.file_pattern, args.case_sensitive, results, this.config.workspace_root);
      return { pattern: args.pattern, matches: results, count: results.length };
    });

    this._registerTool('glob_pattern', TOOL_CATEGORIES.FILE_SYSTEM, '使用 glob 模式匹配文件', {
      type: 'object',
      properties: {
        pattern: { type: 'string' },
        dir_path: { type: 'string', default: '.' }
  ...
  _registerBashTools() {
    this._registerTool('bash_execute', TOOL_CATEGORIES.BASH, '执行 Bash 命令', {
      type: 'object',
      properties: {
        command: { type: 'string' },
        cwd: { type: 'string' },
      ...
      return new Promise((resolve) => {
        const cwd = args.cwd ? path.resolve(this.config.workspace_root, args.cwd) : this.config.workspace_root;
        exec(args.command, { cwd, timeout: args.timeout || this.config.timeout, maxBuffer: this.config.max_output_size }, (error, stdout, stderr) => {
          resolve({
    ...
    this._registerTool('bash_script', TOOL_CATEGORIES.BASH, '执行 Bash 脚本文件', {
      type: 'object',
      properties: {
    ...
    this._registerTool('bash_pipe', TOOL_CATEGORIES.BASH, '使用管道执行多个命令', {
      type: 'object',
      properties: {
    ...
    this._registerTool('bash_background', TOOL_CATEGORIES.BASH, '后台执行命令', {
      type: 'object',
      properties: {
        command: { type: 'string' },
        cwd: { type: 'string' }
    ...
    this._registerTool('bash_background_result', TOOL_CATEGORIES.BASH, '获取后台进程输出', {
      type: 'object',
      properties: {
        pid: { type: 'number' }
      },
    ...
    this._registerTool('get_running_processes', TOOL_CATEGORIES.BASH, '获取运行中的进程列表', {
      type: 'object',
      properties: {
        filter: { type: 'string' }
      }
      ...
      const result = await this.execute('bash_execute', { command: `ps aux${filter} | head -20` });
      return result;
    });

    this._registerTool('kill_process', TOOL_CATEGORIES.BASH, '终止进程', {
      type: 'object',
      properties: {
        pid: { type: 'number' },
      ...
      const result = await this.execute('bash_execute', { command: `kill ${signal} ${args.pid}` });
      this.bashProcesses.delete(args.pid);
      return result;
    });

    this._registerTool('bash_stream', TOOL_CATEGORIES.BASH, '流式执行命令（支持实时输出）', {
      type: 'object',
      properties: {
        command: { type: 'string' },
        cwd: { type: 'string' }
      ...
      return new Promise((resolve) => {
        ...
        proc.stdout.on('data', (data) => { outputs.stdout += data.toString(); });
        proc.stderr.on('data', (data) => { outputs.stderr += data.toString(); });
  ...
  _registerGitTools() {
    this._registerTool('git_status', TOOL_CATEGORIES.GIT, '获取 Git 状态', {
      type: 'object',
      properties: {
        cwd: { type: 'string' }
      }
    ...
    this._registerTool('git_commit', TOOL_CATEGORIES.GIT, '提交更改', {
      type: 'object',
      properties: {
        message: { type: 'string' },
        cwd: { type: 'string' }
      ...
      return this.execute('bash_execute', { command: `git add -A && git commit -m "${args.message.replace(/"/g, '\\"')}"`, cwd: args.cwd });
    });

    this._registerTool('git_push', TOOL_CATEGORIES.GIT, '推送到远程仓库', {
      type: 'object',
      properties: {
        branch: { type: 'string' },
        cwd: { type: 'string' }
    ...
    this._registerTool('git_pull', TOOL_CATEGORIES.GIT, '从远程拉取更新', {
      type: 'object',
      properties: {
        branch: { type: 'string' },
        cwd: { type: 'string' }
    ...
    this._registerTool('git_branch', TOOL_CATEGORIES.GIT, '管理 Git 分支', {
      type: 'object',
      properties: {
      ...
      switch (args.action) {
        ...
        case 'create': cmd = `git branch ${args.name}`; break;
        case 'delete': cmd = `git branch -d ${args.name}`; break;
        case 'switch': cmd = `git checkout ${args.name}`; break;
        default: throw new Error(`Unknown action: ${args.action}`);
      }
      return this.execute('bash_execute', { command: cmd, cwd: args.cwd });
    });

    this._registerTool('git_diff', TOOL_CATEGORIES.GIT, '查看差异', {
      type: 'object',
      properties: {
        target: { type: 'string' },
        cwd: { type: 'string' }
  ...
  _registerPackageManagerTools() {
    this._registerTool('npm_install', TOOL_CATEGORIES.PACKAGE_MANAGER, '安装 npm 依赖', {
      type: 'object',
      properties: {
        package: { type: 'string' },
        flags: { type: 'string' },
      ...
      return this.execute('bash_execute', { command: `npm install ${flags} ${pkg}`, cwd: args.cwd });
    });

    this._registerTool('npm_run', TOOL_CATEGORIES.PACKAGE_MANAGER, '运行 npm 脚本', {
      type: 'object',
      properties: {
        script: { type: 'string' },
        cwd: { type: 'string' }
    ...
    this._registerTool('yarn_install', TOOL_CATEGORIES.PACKAGE_MANAGER, '安装 yarn 依赖', {
      type: 'object',
      properties: {
        package: { type: 'string' },
        cwd: { type: 'string' }
    ...
    this._registerTool('pnpm_install', TOOL_CATEGORIES.PACKAGE_MANAGER, '安装 pnpm 依赖', {
      type: 'object',
      properties: {
        package: { type: 'string' },
        cwd: { type: 'string' }
  ...
  _registerNetworkTools() {
    this._registerTool('http_get', TOOL_CATEGORIES.NETWORK, '发送 GET 请求', {
      type: 'object',
      properties: {
        url: { type: 'string' },
        headers: { type: 'object' }
    ...
    this._registerTool('http_post', TOOL_CATEGORIES.NETWORK, '发送 POST 请求', {
      type: 'object',
      properties: {
        url: { type: 'string' },
        body: { type: 'string' },
    ...
    this._registerTool('http_fetch_json', TOOL_CATEGORIES.NETWORK, '获取并解析 JSON', {
      type: 'object',
      properties: {
        url: { type: 'string' },
        headers: { type: 'object' }
    ...
    this._registerTool('http_download', TOOL_CATEGORIES.NETWORK, '下载文件', {
      type: 'object',
      properties: {
        url: { type: 'string' },
        dest: { type: 'string' }
  ...
  async httpDownload(url, destPath) {
    return this.execute('http_download', { url, dest: path.relative(this.config.workspace_root, destPath) });
  }

  _registerCodeEditTools() {
    this._registerTool('edit_file', TOOL_CATEGORIES.CODE_EDIT, '编辑文件（整体替换）', {
      type: 'object',
      properties: {
        file_path: { type: 'string' },
        content: { type: 'string' }
    ...
    this._registerTool('insert_content', TOOL_CATEGORIES.CODE_EDIT, '在文件指定位置插入内容', {
      type: 'object',
      properties: {
        file_path: { type: 'string' },
        content: { type: 'string' },
    ...
    this._registerTool('remove_content', TOOL_CATEGORIES.CODE_EDIT, '删除文件指定行', {
      type: 'object',
      properties: {
        file_path: { type: 'string' },
    ...
    this._registerTool('replace_content', TOOL_CATEGORIES.CODE_EDIT, '替换文件中的文本', {
      type: 'object',
      properties: {
        file_path: { type: 'string' },
        search: { type: 'string' },
  ...
  _registerLspTools() {
    this._registerTool('lsp_hover', TOOL_CATEGORIES.LSP, 'LSP 悬停信息查询', {
      type: 'object',
      properties: {
        file_path: { type: 'string' },
        line: { type: 'number' },
      ...
      return this.lspRequest(args.language_server, 'textDocument/hover', {
        textDocument: { uri: `file://${path.resolve(this.config.workspace_root, args.file_path)}` },
        position: { line: args.line, character: args.character }
      });
    });

    this._registerTool('lsp_definition', TOOL_CATEGORIES.LSP, 'LSP 跳转定义', {
      type: 'object',
      properties: {
        file_path: { type: 'string' },
        line: { type: 'number' },
    ...
    this._registerTool('lsp_diagnostics', TOOL_CATEGORIES.LSP, 'LSP 诊断信息', {
      type: 'object',
      properties: {
        file_path: { type: 'string' },
        language_server: { type: 'string', default: 'typescript' }
    ...
    this._registerTool('lsp_initialize', TOOL_CATEGORIES.LSP, '初始化 LSP 服务器', {
      type: 'object',
      properties: {
  ...
  _registerSubagentTools() {
    this._registerTool('spawn_subagent', TOOL_CATEGORIES.SUBAGENT, '生成并启动子代理', {
      type: 'object',
      properties: {
        name: { type: 'string' },
        role: { type: 'string' },
    ...
    this._registerTool('delegate_task', TOOL_CATEGORIES.SUBAGENT, '委托任务给子代理', {
      type: 'object',
      properties: {
    ...
    this._registerTool('list_subagents', TOOL_CATEGORIES.SUBAGENT, '列出所有子代理', {
      type: 'object',
      properties: {}
    }, async () => {
      const agents = [];
    ...
    this._registerTool('terminate_subagent', TOOL_CATEGORIES.SUBAGENT, '终止子代理', {
      type: 'object',
      properties: {
        subagent_id: { type: 'string' }
      },
  ...
  _registerTaskTools() {
    this._registerTool('task_create', TOOL_CATEGORIES.TASK, '创建新任务', {
      type: 'object',
      properties: {
        title: { type: 'string' },
    ...
    this._registerTool('task_get', TOOL_CATEGORIES.TASK, '获取任务详情', {
      type: 'object',
      properties: {
        task_id: { type: 'string' }
      },
    ...
    this._registerTool('task_list', TOOL_CATEGORIES.TASK, '列出所有任务', {
      type: 'object',
      properties: {
    ...
    this._registerTool('task_update', TOOL_CATEGORIES.TASK, '更新任务状态', {
      type: 'object',
      properties: {
        task_id: { type: 'string' },
    ...
    this._registerTool('task_stop', TOOL_CATEGORIES.TASK, '停止任务', {
      type: 'object',
      properties: {
        task_id: { type: 'string' }
      },
    ...
    this._registerTool('task_output', TOOL_CATEGORIES.TASK, '获取任务输出', {
      type: 'object',
      properties: {
        task_id: { type: 'string' }
      },
    ...
    this._registerTool('task_delete', TOOL_CATEGORIES.TASK, '删除任务', {
      type: 'object',
      properties: {
        task_id: { type: 'string' }
      },
  ...
  _registerWorkflowTools() {
    this._registerTool('enter_plan_mode', TOOL_CATEGORIES.WORKFLOW, '进入规划模式', {
      type: 'object',
      properties: {
        context: { type: 'object' }
      }
    ...
    this._registerTool('exit_plan_mode', TOOL_CATEGORIES.WORKFLOW, '退出规划模式', {
      type: 'object',
      properties: {}
    }, async () => {
    ...
    this._registerTool('enter_worktree', TOOL_CATEGORIES.WORKFLOW, '创建并进入工作树', {
      type: 'object',
      properties: {
        name: { type: 'string' },
    ...
    this._registerTool('exit_worktree', TOOL_CATEGORIES.WORKFLOW, '退出工作树', {
      type: 'object',
      properties: {
    ...
    this._registerTool('todo_write', TOOL_CATEGORIES.WORKFLOW, '写入任务清单', {
      type: 'object',
      properties: {
        todos: {
          type: 'array',
    ...
    this._registerTool('cron_schedule', TOOL_CATEGORIES.WORKFLOW, '创建定时任务', {
      type: 'object',
      properties: {
    ...
    this._registerTool('cron_list', TOOL_CATEGORIES.WORKFLOW, '列出定时任务', {
      type: 'object',
      properties: {}
    }, async () => {
    ...
    this._registerTool('cron_delete', TOOL_CATEGORIES.WORKFLOW, '删除定时任务', {
      type: 'object',
      properties: {
        job_id: { type: 'string' }
      },
  ...
  _registerMcpTools() {
    this._registerTool('mcp_list_tools', TOOL_CATEGORIES.MCP, '列出 MCP 服务器上的工具', {
      type: 'object',
      properties: {
      ...
      try {
        ...
        return { server: args.server_name, tools: tools.tools || [], count: tools.tools?.length || 0 };
      } catch (err) {
        return { error: err.message };
      }
    });

    this._registerTool('mcp_read_resource', TOOL_CATEGORIES.MCP, '读取 MCP 资源', {
      type: 'object',
      properties: {
        server_name: { type: 'string' },
        uri: { type: 'string' }
    ...
    this._registerTool('mcp_call_tool', TOOL_CATEGORIES.MCP, '调用 MCP 工具', {
      type: 'object',
      properties: {
        server_name: { type: 'string' },
    ...
    this._registerTool('mcp_list_servers', TOOL_CATEGORIES.MCP, '列出所有 MCP 服务器', {
      type: 'object',
      properties: {}
    }, async () => {
  ...
  _registerAgentTools() {
    this._registerTool('send_message', TOOL_CATEGORIES.AGENT, '向代理发送消息', {
      type: 'object',
      properties: {
    ...
    this._registerTool('team_create', TOOL_CATEGORIES.AGENT, '创建代理团队', {
      type: 'object',
      properties: {
        name: { type: 'string' },
        agents: {
    ...
    this._registerTool('team_delete', TOOL_CATEGORIES.AGENT, '删除代理团队', {
      type: 'object',
      properties: {
        team_id: { type: 'string' }
      },
    ...
    this._registerTool('team_list', TOOL_CATEGORIES.AGENT, '列出所有代理团队', {
      type: 'object',
      properties: {}
    }, async () => {
    ...
    this._registerTool('agent_status', TOOL_CATEGORIES.AGENT, '获取代理状态', {
      type: 'object',
      properties: {
        agent_id: { type: 'string' }
      },
  ...
  _registerInteractiveTools() {
    this._registerTool('ask_user_question', TOOL_CATEGORIES.INTERACTIVE, '向用户提问', {
      type: 'object',
      properties: {
    ...
    this._registerTool('notebook_edit', TOOL_CATEGORIES.FILE_SYSTEM, '编辑 Jupyter Notebook', {
      type: 'object',
      properties: {
        file_path: { type: 'string' },
    ...
    this._registerTool('powershell_execute', TOOL_CATEGORIES.BASH, '执行 PowerShell 命令', {
      type: 'object',
      properties: {
        command: { type: 'string' },
        cwd: { type: 'string' },
      ...
      return new Promise((resolve) => {
        const cwd = args.cwd ? path.resolve(this.config.workspace_root, args.cwd) : this.config.workspace_root;
        const psCommand = args.command.includes('powershell') ? args.command : `powershell -Command "${args.command.replace(/"/g, '\\"')}"`;
        exec(psCommand, { cwd, timeout: args.timeout || this.config.timeout }, (error, stdout, stderr) => {
          resolve({
            command: args.command,
            stdout: stdout.substring(0, this.config.max_output_size),
            stderr: stderr.substring(0, this.config.max_output_size),
    ...
    this._registerTool('web_fetch', TOOL_CATEGORIES.NETWORK, '获取网页内容', {
      type: 'object',
      properties: {
        url: { type: 'string' }
      },
    ...
    this._registerTool('web_search', TOOL_CATEGORIES.NETWORK, '网络搜索', {
      type: 'object',
      properties: {
        query: { type: 'string' },
    ...
    this._registerTool('tool_search', TOOL_CATEGORIES.UTILITY, '搜索可用工具', {
      type: 'object',
      properties: {
        query: { type: 'string' },
      ...
      const results = allTools.filter(t => {
        const matchQuery = t.name.toLowerCase().includes(query) || t.description.toLowerCase().includes(query);
  ...
  _registerUtilityTools() {
    this._registerTool('regex_search', TOOL_CATEGORIES.UTILITY, '正则表达式搜索', {
      type: 'object',
      properties: {
        pattern: { type: 'string' },
    ...
    this._registerTool('json_parse', TOOL_CATEGORIES.UTILITY, '解析 JSON', {
      type: 'object',
      properties: {
        content: { type: 'string' }
      },
    ...
    this._registerTool('yaml_parse', TOOL_CATEGORIES.UTILITY, '解析 YAML', {
      type: 'object',
      properties: {
        content: { type: 'string' }
      },
    ...
    this._registerTool('template_render', TOOL_CATEGORIES.UTILITY, '模板渲染', {
      type: 'object',
      properties: {
